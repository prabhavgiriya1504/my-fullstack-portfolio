...
// // 🛑 THE CRITICAL FIX: Forces the route to render dynamically on the server
...
export default async function AdminContactPage() {
    ...
    // ===========================================
    // 🔍 DEBUG STEP 1: Check if the token was read
    // ===========================================
    if (!adminToken) {
    ...
    try {
        ...
        // =======================================================
        // 🔍 DEBUG STEP 2: Check for 401/403 status from the backend
        // =======================================================
        if (res.status === 401 || res.status === 403) {
        ...
        if (!res.ok) {
            console.error(`⚠️ [AdminContactPage] Backend failed to fetch contacts: ${res.status}. Response OK is false.`);
